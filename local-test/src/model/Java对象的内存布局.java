...
//显式调用又可分为两种，一是直接使用“super”关键字调用父类构造器，二是使用“this”关键字调用同一个类中的其他构造器。
...
//为了尽量较少对象的内存使用量，64 位 Java 虚拟机引入了压缩指针https://wiki.openjdk.java.net/display/HotSpot/CompressedOops
//的概念（对应虚拟机选项 -XX:+UseCompressedOops，默认开启），将堆中原本 64 位的 Java 对象指针压缩成 32 位的。
...
//原本的内存寻址用的是车位号。比如说我有一个值为 6 的指针，代表第 6 个车位，那么沿着这个指针可以找到 3 号车。现在我们规定指针里存的值是车号，比如 3 指代 3 号车。
//当需要查找 3 号车时，我便可以将该指针的值乘以 2，再沿着 6 号车位找到 3 号车。
...
//此外，我们可以通过配置刚刚提到的内存对齐选项（-XX:ObjectAlignmentInBytes）来进一步提升寻址范围。但是，这同时也可能增加填充，导致压缩指针没有达到原本节省空间的效果。
//举例来说，如果规定每辆车都需要从偶数车位号停起，那么对于占据两个车位的小房车来说，刚刚好，而对于需要三个车位的大房车来说，也仅是浪费一个车位。
//但是如果规定需要从4的倍数号车位停起，那么小房车则会浪费两个车位，而大房车至多可能浪费三个车位。

//当然，就算是关闭了压缩指针，java虚拟机还是会进行内存对齐。此外，内存对齐不仅存在于对象和对象之间，也存在于对象中的字段之间。比如说，Java虚拟机要求long字段、double字段，
//以及非压缩指针状态下的引用字段地址为8的倍数。

//字段内存对齐的其中一个原因，是让字段只出现在同一cpu的缓存杭中。如果字段不是对齐的，那么就有可能出现跨缓存行的字段。也就是说，该字段的读取可能需要替换两个缓存行，
//而该字段的存储也会同时污染两个缓存行。这两种情况对程序的执行效率而言都是不利的。

//下面介绍下对象内存布局另一个有趣的特性：字段重排序。

//http://openjdk.java.net/jeps/142
二、字段重排列
//字段重排序，顾名思义，就是在Java虚拟机重新分配字段的先后顺序，以达到内存对齐的目的。java虚拟机有三种排列方法（对应Java虚拟机选项-XX：FieldAllocationStyle,默认值为1），但都会遵循如下两个原则。
//其一，如果一个字段占据C个字节，那么该字段的偏移量需要对齐志NC。这里偏移量指的是字段地址与对象的起始地址差值。
//以long类为例，它仅有一个long类型的实例字段。在使用了压缩指针的64位虚拟机中，尽量对象头的大小为12字节，该long类型字段的偏移量也只能是16，而中间空着的4个字节便会被浪费掉。
...
//上述代码里边定义了两个类A和B，其中B继承A。A和B各自定义了一个long类型的实例字段和一个int类型的实例字段。下面分别打印了B类在启用压缩指针和未启用压缩指针时，各个字段的偏移量。
	# 启用压缩指针时，B 类的字段分布
	B object internals:
...
//当启用压缩指针时，可以看到java虚拟机将A类的int字段放置于long字段之前，以填充因为long字段对齐造成的4字节缺口。由于对象整体大虚需要对齐至8N，因此对象的最后会有4字节的空白填充。
	     # 关闭压缩指针时，B 类的字段分布
	     B object internals:
	      OFFSET  SIZE   TYPE DESCRIPTION
	           0     4        (object header)
...
//Java 8还引入了一个新的注释@Contended，用来解决对象字段之间的虚共享问题http://openjdk.java.net/jeps/142，这个注释也会影响到字段的排列。
	          
//虚共享是怎么回事呢？假设两个线程分别访问同一对象中不同的volatile字段，逻辑上他们并没有共享内容，因此不需要同步。
	          
//然而，如果这两个字段恰好在同一个缓存行中，那么对这些字段的写操作会导致缓存行的写回，也就造成了实质上的共享。
	          
//Java虚拟机会让不同的@Contended字段处于独立的缓存行中，因此你会看到大量的空间被浪费掉。
//如果你感兴趣，可以利用实践环节的工具，来查阅Contended字段的内存布局。注意使用虚拟机选项-XX:-RestrictContended。
//如果你在Java9以上版本试验的话，在使用javac编译时需要加上--add-exports java.base/jdk.internal.vm.annotation=ALL_UNNAME	          
	          

三、总结和实践
//今天我介绍了java虚拟机构造对象的方式，所构造对象的大小，以及对象的内存分布。

//常见的new语句会被编译为new指令，以及对构造器的调用。每个类的构造器皆会直接或者间接调用父类的构造器，并且在同一个实例中初始化相应的字段。

//Java虚拟机引入了压缩指针的概念，将原本64位的指针压缩成的32位。压缩指针要求java虚拟机堆中对象的起始地址要对齐至8的倍数。
//java虚拟机还会对每个类的字段进行重排序，使得字段也能够内存对齐。
